...
For text-generation calls, `transport = "openai"` can also use ChatGPT Codex subscription OAuth instead of an OpenAI API key. Set `MODEL_CONFIG__OVERRIDES__AUTH_MODE=codex_oauth` on the relevant Deriver, Dialectic, Summary, or Dream model config. Honcho reads Codex CLI tokens from `$CODEX_HOME/auth.json` or `~/.codex/auth.json`. Embeddings still need a configured embedding provider key.
